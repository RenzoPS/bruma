...
.claude/       this file, plus specs/ (the two original design docs)
...
- **`apps/web/AGENTS.md` is generated by `next dev`** and re-added on every run.
  It is committed on purpose: removing it only recreates an uncommitted change.
  `apps/web/CLAUDE.md` is a one-line `@AGENTS.md` import of it.
- **Next 16 differs from older versions.** Read
